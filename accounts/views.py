...
def registerUser(request):
    if request.user.is_authenticated:
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your account has been registered successfully! Please, check your email inbox for activation!')
...
def login(request):
    if request.user.is_authenticated:
        ...
        if user is not None:
            ...
            messages.error(request, "Invalid email or/and password")
...
def vendorDashboard(request):
    ...
    for i in current_month_orders:
        current_month_revenue += int(i.get_total_by_vendor())

    # # total revenue per vendor
    total_revenue = 0
    for i in orders:
        total_revenue += int(i.get_total_by_vendor())
